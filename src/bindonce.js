/**
 * @license Bindonce v0.3.3 - Zero watches binding for AngularJs
...
bindonceModule.directive('bindonce', function () {
  ...
  return {
    ...
    controller: function ($scope, $element, $attrs, $interpolate) {
      ...
      var transcludeSelection = function (selectedTransclude) {
        transclude(selectedTransclude, this.scope.$new());
      };

      var setBinderAttr = function (attrValue, attrKey) {
        var newAttr, newValue;
        if (attrKey.match(/^boAttr./) && this.attrs[attrKey]) {
          newAttr = attrKey.replace(/^boAttr/, '').replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
          newValue = this.scope.$eval(this.attrs[attrKey]);
          this.element.attr(newAttr, newValue);
        }
      };
            ...
            switch (binder.attr) {
              ...
              case 'boSwitch':
                ...
                if ((selectedTranscludes = switchCtrl.cases['!' + value] || switchCtrl.cases['?'])) {
                  ...
                  angular.forEach(selectedTranscludes, transcludeSelection, binder);
                }
                break;
              case 'boSwitchWhen':
                var switchWhenCtrl = binder.controller[0];
                switchWhenCtrl.cases['!' + binder.attrs.boSwitchWhen] = (switchWhenCtrl.cases['!' + binder.attrs.boSwitchWhen] || []);
                switchWhenCtrl.cases['!' + binder.attrs.boSwitchWhen].push({
                  transclude: binder.transclude,
                  element: binder.element
                });
                break;
              case 'boSwitchDefault':
                var switchDefaultCtrl = binder.controller[0];
                switchDefaultCtrl.cases['?'] = (switchDefaultCtrl.cases['?'] || []);
                switchDefaultCtrl.cases['?'].push({transclude: binder.transclude, element: binder.element});
                break;
              case 'hide':
              case 'show':
              ...
              case 'src':
                ...
                break;
              case 'attr':
                angular.forEach(binder.attrs, setBinderAttr, binder);
                break;
              case 'href':
              case 'alt':
              case 'title':
